...
//objeto que necesito en formato de base de datos
let BaseDeDatos={
	title:"",
	summary:"",
	healthScore:0,
...
//---------------------------------------------------------------------------------	
let arr= [{a:1, b:2, c:["aa","bb"]}, {a:3, b:4, c:["dd","ee"]}]
const func=(ob)=>{
	let newArr=[]
...
console.log(arr[0].c[0])


const changeObject0=(ob)=>{
	let newArr=[]
	for (let i=0; i<ob.length; i++){
		let newObj={title:"", summary:"",healthScore:0,analyzedInstructions:"", image:""}
		for (let prop in ob[i]){
			if (prop==="title"){
...
console.log(changeObject0(obj.results))
console.log(obj.results[0])

const changeObject2 = (ob) => {
	let newArr = [];
	for (let i = 0; i < ob.length; i++) {
		let newObj = {
			title: "",
		...
		for (let prop in ob[i]) {
			...
			//let dishTypes= ["lunch","main course", "main dish", "dinner","morning meal", "brunch","breakfast","side dish","soup","other"]
			if (prop === "dishTypes") {
...
console.log(changeObject2(obj.results))

const tipos = (objeto)=>{
	let res=[]
	for (let i=0; i<objeto.length; i++){
		console.log(objeto[i])
		for (let prop in objeto[i]){
			console.log(prop)
			if (prop==="dishTypes"){
				res.push(objeto[i].dishTypes)	
			}
		}
	}
	return res
}
console.log(tipos(obj.results))


let dishTypes= ["lunch","main course", "main dish", "dinner","morning meal", "brunch","breakfast","side dish","soup","other"]
let reduced= dishTypes.reduce((acc, item)=>{
	return acc=acc+", "+ item
})
console.log(reduced)

console.log(obj.results[0].analyzedInstructions[0].steps[0])
console.log(obj.results[0].analyzedInstructions[0])
console.log(obj.results.flatMap((el)=>el.analyzedInstructions))
console.log(obj.results.flatMap((el)=>el.analyzedInstructions.flatMap((ele)=>ele.steps)))

//NEXT STEPS!!!
let data=obj.results[0].analyzedInstructions.flatMap((el)=>el.steps.flatMap((e)=>e.step))
console.log(data.reduce((acc,item)=>acc+=item))


let arrflat=[
	{
		a:"a",
		b: [{d: "step", e: 2, f: 3}, {d:"step", e: 2, f: 3}],
		c: "c"
	},
	{
		a:"a",
		b: [{d: "step", e: 3, f: 4}, {d: "step", e: 6, f: 7}],
		c: "c"
	},
{
		a:"a",
...
let a= arrflat.flatMap((el)=>el.b)
let b= a.flatMap((el)=>el.d)
console.log(b)
let c= arrflat.flatMap((el)=>el.b.flatMap((e)=>e.d))
console.log(c)
...
function arrayNoObject (arr){
	let res=[]
	for (let i=0; i<arr.length; i++){
		for (let prop in arr[i]){
			console.log(arr[i].b[0].d)
			if (prop==="b"){
				for (let j=0; j<arr[i].b.length; j++){
					res.push(arr[i].b[j].d)
				}
			}
		}
	}
...
console.log(arrayNoObject(arrflat))
